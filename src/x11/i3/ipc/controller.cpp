...
void I3IpcController::setFocusedMonitor(I3Monitor* monitor) {
	...
	if (oldMonitor != nullptr) { QObject::disconnect(oldMonitor, nullptr, this, nullptr); }
...
void I3IpcController::handleGetWorkspacesEvent(I3IpcEvent* event) {
	...
	for (auto entry: workspaces) {
		...
		if (workspace == nullptr) { workspace = new I3Workspace(this); }
...
void I3IpcController::handleGetOutputsEvent(I3IpcEvent* event) {
	...
	for (auto elem: monitors) {
		...
		if (monitor == nullptr) { monitor = new I3Monitor(this); }

		monitor->updateFromObject(object);

		if (monitor->bindableFocused().value()) { this->setFocusedMonitor(monitor); }

		if (!existed) { this->mMonitors.insertObject(monitor); }
...
void I3IpcController::handleWorkspaceEvent(I3IpcEvent* event) {
	...
	if (change == "init") {
		...
		if (!existed) { workspace = new I3Workspace(this); }
		...
		if (newWorkspace == nullptr) { newWorkspace = new I3Workspace(this); }
